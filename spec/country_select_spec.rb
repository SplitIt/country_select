    ...
    describe CountrySelect do
      ...
      let(:selected_us_option) do
        if defined?(Tags::Base)
          content_tag(:option, 'United States', :selected => :selected, :value => 'usa')
        else
      ...
      describe "#priority_countries" do
        let(:tag) { builder.country_select(:country_name, ['usa']) }

        it "puts the countries at the top" do
          tag.should include("#{select_tag}<option value=\"usa")
        end
